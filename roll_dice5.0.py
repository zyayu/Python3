...
def main():
    ...
    roll_result_arr = roll_1_arr + roll_2_arr
    hist, bins = np.histogram(roll_result_arr, bins=range(2, 13))
    print(hist)
    print(bins)
    # 数据可视化 分布直方图
    plt.hist(roll_result_arr, bins=range(2, 14), density=1, edgecolor='white', linewidth=1, rwidth=0.8)  # density 归一化
    # 坐标值的现实
